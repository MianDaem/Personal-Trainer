...
def apply_styles() -> None:
    st.markdown(
        ...
        .stApp {
            background:
                radial-gradient(circle at 0% 0%, rgba(56, 189, 248, 0.20), transparent 24%),
                radial-gradient(circle at 100% 0%, rgba(249, 115, 22, 0.18), transparent 22%),
                radial-gradient(circle at 50% 100%, rgba(52, 211, 153, 0.15), transparent 28%),
                linear-gradient(180deg, #050b16 0%, #0a1220 38%, #111c30 100%);
            color: #e5eef8;
        }
        [data-testid="stSidebar"] {
            background:
                linear-gradient(180deg, rgba(6, 11, 22, 0.98) 0%, rgba(12, 20, 36, 0.96) 100%);
            border-right: 1px solid rgba(255, 255, 255, 0.08);
        }
        .hero-card, .glass-card, .plan-card, .auth-card {
            background:
                linear-gradient(145deg, rgba(15, 23, 42, 0.86) 0%, rgba(18, 34, 56, 0.78) 100%);
        ...
        .hero-card::before {
            ...
            inset: 0;
            background:
                linear-gradient(115deg, rgba(56, 189, 248, 0.10), transparent 35%, rgba(249, 115, 22, 0.08));
            pointer-events: none;
        }
        .muted {
            color: #a8bad6;
        ...
        .section-shell {
            background: linear-gradient(145deg, rgba(14, 20, 38, 0.88), rgba(14, 28, 43, 0.72));
        ...
        .stButton > button {
            ...
            background: linear-gradient(135deg, #17324a 0%, #1b6758 52%, #f97316 130%);
            color: #f8fafc;
            font-weight: 600;
            min-height: 2.95rem;
            box-shadow: 0 14px 32px rgba(8, 15, 28, 0.28);
        ...
        .stTabs [aria-selected="true"] {
            background: linear-gradient(135deg, rgba(56, 189, 248, 0.22), rgba(249, 115, 22, 0.18)) !important;
        ...
        div[data-testid="stMetric"] {
            background: linear-gradient(145deg, rgba(15, 23, 42, 0.82), rgba(18, 31, 52, 0.72));
    ...
    with center:
        st.markdown(
            ...
            <div class="auth-card">
                ...
                <p class="muted">A vibrant personal trainer experience with separate dashboards, saved goals, meal tracking, workouts, and progress for every user.</p>
...
def workouts_tab(exercise_df: pd.DataFrame) -> None:
    ...
    st.markdown(
        ...
        <div class="glass-card" style="margin-bottom:1rem;">
            <h3 style="margin-bottom:0.35rem;">Workout Builder</h3>
            <p class="muted">Shape a session around your target muscles, available equipment, and current goal. The trainer prescription updates live as you filter.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def nutrition_tab(nutrition_df: pd.DataFrame, targets: dict) -> None:
    st.markdown(
        """
        <div class="glass-card" style="margin-bottom:1rem;">
            <h3 style="margin-bottom:0.35rem;">Nutrition Coach</h3>
            <p class="muted">Search real foods, calculate portions instantly, and keep your macros lined up with the training goal.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
...
def progress_tab(targets: dict) -> None:
    st.markdown(
        """
        <div class="glass-card" style="margin-bottom:1rem;">
            <h3 style="margin-bottom:0.35rem;">Progress Tracker</h3>
            <p class="muted">Log body changes, energy, and recovery notes so the app feels like a coach, not just a calculator.</p>
        </div>
        """,
        unsafe_allow_html=True,
    )
